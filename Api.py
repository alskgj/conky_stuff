...
class ConkyWriter:
    ...
    def writeCommand(self, command, parameters=None):
        """
        ...
        if type(parameters) is list:
            ...
            parameters = ' '.join(filtered)
        elif type(parameters) is tuple:
            if None in parameters:
                raise ValueError('Can not write None value from tuple')
            parameters = ','.join(parameters)
        elif type(parameters) is None:
            parameters = ""
        self.write('${%s %s}' % (command, parameters))
        return self
    ...
    def rss(self, uri, interval, action, numPar=None, space=None):
        ...
        """
        parameters = [uri, interval, action]
